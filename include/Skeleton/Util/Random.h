...
namespace sen {
    ...
    class Random {
      ...
      public:
         ...
         */
        template <typename T>
        static typename std::enable_if<std::is_floating_point<T>::value, T>::type get(T min, T max) {
            std::uniform_real_distribution<T> d(min, max);
            return d(s_engine);
        }

        template <typename T> static typename std::enable_if<std::is_integral<T>::value, T>::type get(T min, T max) {
